...
void wavefunction::initplane(grid g, int inittype, double width, double time)
{
...
alpha=50;
double energy;
energy=0.5385;//k*k*0.5;



if((x>0))
start[g.index(xindex,yindex,0)]=exp(-imagi*k*x);//+exp(-imagi*k*x*0.4975))*exp(-imagi*energy*time);//*wf[xindex];

if((y>0))

start[g.index(yindex,yindex,0)]=exp(-imagi*k*y);//+exp(-imagi*k*y*0.4975))*exp(-imagi*energy*time);//*wfone[yindex];
...
if((x<0))
start[g.index(xindex,xindex,0)]=exp(imagi*k*x);//+exp(-imagi*k*x*0.4975))*exp(-imagi*energy*time);//*wf[xindex];
...
void wavefunction::initwf(grid g, int inittype, double box, double k, double time,const wavefunction &wf)
{
  long xindex, yindex, zindex, index_i;
  double x,y,z;
  dcomplex st,imagi;
...
energy=0.5385;//k*k*0.5;
	  ...
	  for (yindex=0; yindex<g.offs_y()-box; yindex++)
	    {
			...
			y=g.y(yindex);
	      index_i=g.index(xindex,yindex,0);
	       start[index_i]=exp(-imagi*k*y);
 ...
 for (xindex=g.offs_x()-box; xindex<g.offs_x()+box; xindex++)
        {
          for (yindex=g.offs_x()-box; yindex<g.offs_y()+box; yindex++)
            {

                        y=g.y(yindex);
              index_i=g.index(xindex,yindex,0);
               start[index_i]=wf[index_i];
...
void wavefunction::initwfeven(grid g, int inittype, double box, double k, double time)
{
  long xindex, yindex, zindex, index_i;
  double x,y,z;
  dcomplex st,imagi;
	  ...
	  for (yindex=0; yindex<g.ngps_y(); yindex++)
	    {
			...
			y=g.y(yindex);
	      index_i=g.index(xindex,yindex,0);
	       start[index_i]=sin(219*3.145*yindex/g.ngps_y())*exp(alpha*x*x);
	  ...
	  for (xindex=0; xindex<g.ngps_x(); xindex++)
	    {
			...
			y=g.y(yindex);
	      index_i=g.index(xindex,yindex,0);
	       start[index_i]=sin(219*3.145*xindex/g.ngps_x())*exp(alpha*y*y);
          ...
          for (yindex=g.offs_x()-box; yindex<g.offs_y()+box; yindex++)
            {
                        ...
                        y=g.y(yindex);
              index_i=g.index(xindex,yindex,0);
               start[index_i]=sin(219*3.145*x/g.ngps_x())*exp(alpha*y*y)+sin(219*3.145*y/g.ngps_y())*exp(alpha*x*x);
...
void wavefunction::initwfodd(grid g, int inittype, double box, double k, double time,const wavefunction &wf)
{
  long xindex, yindex, zindex, index_i;
  double x,y,z;
  dcomplex st,imagi;
	  ...
	  for (yindex=0; yindex<g.offs_y()-box; yindex++)
	    {
			...
			x=g.x(xindex);
	      index_i=g.index(xindex,yindex,0);
	       start[index_i]=0.0*x*exp(-imagi*k*y)*exp(beta*x*x);
	  ...
	  for (xindex=0; xindex<g.offs_x()-box; xindex++)
	    {
			...
			x=g.x(xindex);
	      index_i=g.index(xindex,yindex,0);
		start[index_i]=y*exp(-imagi*k*x)*exp(beta*y*y);
...
wavefunction wavefunction::rotate(grid gone,grid g)
{
long xindex,yindex;


wavefunction result(gone.ngps_x()*gone.ngps_y()*gone.ngps_z());
...
result[gone.index(xindex,yindex,0)]=start[g.index(xindex,yindex,0)];
};
...
void wavefunction::init(grid g, int inittype, double width, double k, double time)
{
  ...
  double x,y,z;
  dcomplex st,imagi;
  double offs=g.ngps_x()/2.0;
   double offsy=g.ngps_y()/2.0;
  imagi=dcomplex(0.0,1.0);
  st=width+0.5*dcomplex(0.0,1.0)*time/width;
  ...
  for (xindex=0; xindex<g.ngps_x(); xindex++)
      ...
      for (yindex=0; yindex<g.ngps_y(); yindex++)
	{
	  ...
	  for (zindex=0; zindex<g.ngps_z(); zindex++)
		...
		{
		case 0 :
		  start[index_i]=exp(-0.01*(x*x+y*y));  //(x+y)*exp(-0.01*x*x-0.01*y*y);
		  break;
		case 1 :
		  start[index_i]= sin(87*3.14*x/(g.ngps_x()*0.5*g.delt_x()))*exp(-width*y*y)+sin(87*3.14*y/(g.ngps_y()*0.5*g.delt_y()))*exp(-width*x*x); //cos(x)*cos(y); //(1/sqrt(2))*(sin(x)*cos(y)+sin(y)*cos(x));//(sin(x)*cos(y)+sin(y)*cos(x)) ;   //cos(x)*cos(y)*exp(-x*x-y*y);//(sin(x)*cos(y)+sin(y)*cos(x))*exp(-x*x-y*y) ;
		  break;
		case 2 :
...
alpha=50.0;
gamma=170.0*170.0;
if((-alpha<x<alpha) && (-alpha<y<alpha) && (y<-x) )


start[index_i]=1.0;//-1.0;
...
/*
if((-alpha<x<alpha) && (-alpha<y<alpha) && (y<-x) && (y>0) )


start[index_i]=1.0;

if((-alpha<x<alpha) && (-alpha<y<alpha) && (y>-x) && (x<0) )
start[index_i]=-1.0;




if((-alpha<x<alpha) && (-alpha<y<alpha) && (x<0) && (y<0) )


start[index_i]=-1.0;

if((-alpha<x<alpha) && (-alpha<y<alpha) && (y>-x) && (x>0) && (y<0) )
start[index_i]=-1.0;
...
if((x>alpha) && y>0)
start[index_i]=exp(-imagi*k*x)*exp(-imagi*energy*time);
...
if((y>alpha) && x>0)
start[index_i]=exp(-imagi*k*y)*exp(-imagi*energy*time);
...
complex<double>  wavefunction::projection(grid gbig,const wavefunction &wfplane)
{
 dcomplex result=dcomplex(0.0,0.0);

 dcomplex rhs=dcomplex(0.0,0.0);
 dcomplex lhs=dcomplex(0.0,0.0);


//wavefunction rhs(gbig.ngps_x()*gbig.ngps_y()*gbig.ngps_z());
double x;


long xindex,yindex,zindex,indexbig,indexbigone;
 for (xindex=0; xindex<gbig.ngps_x(); xindex++)
    {
     for (yindex=0; yindex<gbig.ngps_y(); yindex++)
{
        indexbig=gbig.index(xindex,yindex,0);

      rhs+=conj(start[indexbig])*wfplane[indexbig]*gbig.delt_x()*gbig.delt_y();  //(imagi*(log(start[xindex]))/deltat);
     ...
     for (yindex=0; yindex<gbig.ngps_y(); yindex++)
{
      ...
      lhs+=(wf[indexbig])*wfplane[indexbig]*gbig.delt_x()*gbig.delt_y();  //(imagi*(log(start[xindex]))/deltat);
...
complex<double>  wavefunction::tsurff(grid gbig,grid gbigone,const wavefunction &wf, double k, double posofinterest)
{
 dcomplex result=dcomplex(0.0,0.0);

wavefunction lhs(gbigone.ngps_x()*gbigone.ngps_y()*gbigone.ngps_z());
wavefunction phs(gbigone.ngps_x()*gbigone.ngps_y()*gbigone.ngps_z());
wavefunction lhs_x(gbigone.ngps_x()*gbigone.ngps_y()*gbigone.ngps_z());
wavefunction result_x(gbigone.ngps_x()*gbigone.ngps_y()*gbigone.ngps_z());
wavefunction rhs(gbigone.ngps_x()*gbigone.ngps_y()*gbigone.ngps_z());
double x;
   double aaa, ccc, bbb,aaa2, ccc2, bbb2,aaaupper,cccupper,bbbupper;
  double b_upperleft, b_lowerright,b_upperleft2, b_lowerright2,b_upperleftupper,b_lowerrightupper;
  dcomplex imagi(0.0,1.0);
...
{
        indexbig=gbig.index(xindex,yindex,0);
indexbigone=gbigone.index(xindex,0,0);
      rhs[indexbigone]+=conj(wf[xindex])*start[indexbig]*gbig.delt_x();  //(imagi*(log(start[xindex]))/deltat);

    };
};

  //rhs
  aaa=1.0/(2.0*gbigone.delt_x());
  bbb=0.0;
  ccc=-aaa;
      ...
      lhs[xindex]=aaa*rhs[xindex+1]+lambda/(2.0*gbigone.delt_x())*rhs[xindex];

      for (xindex=1; xindex<gbigone.ngps_x()-1; xindex++)
        {


          lhs[xindex]=aaa*rhs[xindex+1]+bbb*rhs[xindex]+ccc*rhs[xindex-1];
        };

      xindex=gbigone.ngps_x()-1;


      lhs[xindex]=llambda/(2.0*gbigone.delt_x())*rhs[xindex]+ccc*rhs[xindex-1];

  //lhs

      for (xindex=0; xindex<gbigone.ngps_x(); xindex++)
        {

          lhs_x[xindex]=lhs[xindex];
        };
      ...
      result_x.solve_toep(aaa,bbb,b_upperleft,b_lowerright,ccc,lhs_x,gbigone.ngps_x());

      for (xindex=0; xindex<gbigone.ngps_x(); xindex++)
        {

          lhs[xindex]=result_x[xindex];
...
x=gbigone.x(xindex);
phs[xindex]=exp(-imagi*k*x)*lhs[xindex];

};

result=phs[posofinterest];
...
complex<double>  wavefunction::autocorrelationnew(grid gbig,const wavefunction &wf,const wavefunction &wfini)

{
 ...
 wavefunction wfafterlaser(gbig.ngps_x()*gbig.ngps_y()*gbig.ngps_z());

 wavefunction rhs(gbig.ngps_x()*gbig.ngps_y()*gbig.ngps_z());

 wavefunction lhs(gbig.ngps_x()*gbig.ngps_y()*gbig.ngps_z());


  long xindex,yindex,zindex,indexbig,indexauto;
     ...
     for (yindex=0; yindex<gbig.ngps_y(); yindex++)
{
      ...
      rhs[indexbig]=conj(wfini[indexbig])*start[indexbig];  //(imagi*(log(start[xindex]))/deltat);
...
indexbig=gbig.index(xindex,yindex,0);
result+=wf[indexbig]*rhs[indexbig]*gbig.delt_y()*gbig.delt_x();
};
};
...
complex<double>  wavefunction::autocorrelationleinatpt(grid gbig,const wavefunction &wfinibig,const wavefunction &wf,long posinterest)

{
 ...
 wavefunction rhs(gbig.ngps_x()*gbig.ngps_z());



  long xindex,yindex,zindex,indexbig,indexauto,index;
 ...
 for (xindex=0; xindex<gbig.ngps_x(); xindex++)
    ...
    {
                indexbig=gbig.index(xindex,yindex,0);
index=gbig.index(posinterest,yindex,0);

      rhs[xindex]+=conj(wfinibig[indexbig])*start[index]*gbig.delt_y();  //(imagi*(log(start[xindex]))/deltat);
...
complex<double>  wavefunction::autocorrelationsecond(grid gbig,const wavefunction &wf,long posinterest)

{
 ...
 wavefunction rhs(gbig.ngps_y()*gbig.ngps_z());
  ...
  long xindex,yindex,zindex,indexbig,indexauto,index;
     ...
     for (yindex=0; yindex<gbig.ngps_y(); yindex++)
    {
                indexbig=gbig.index(posinterest,yindex,0);
               
      rhs[yindex]=start[indexbig];//wf[indexbig]*start[indexbig];  //(imagi*(log(start[xindex]))/deltat);
