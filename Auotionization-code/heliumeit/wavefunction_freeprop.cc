      ...
      for (yindex=0; yindex<g.ngps_y(); yindex++)
	{
	  ...
	  for (zindex=0; zindex<g.ngps_z(); zindex++)
		...
		case 0 :
		  ...
		  break;
		case 1 :
		  start[index_i]= cos(x)*cos(y); //(1/sqrt(2))*(sin(x)*cos(y)+sin(y)*cos(x));//(sin(x)*cos(y)+sin(y)*cos(x)) ;   //cos(x)*cos(y)*exp(-x*x-y*y);//(sin(x)*cos(y)+sin(y)*cos(x))*exp(-x*x-y*y) ;
		  break;
		case 2 :
...
//{
  //      indexbig=gbig.index(xindex,yindex,0);
   ...
   //   lhs[indexbig]=wfini[indexbig]*wf[indexbig];  //(imagi*(log(start[xindex]))/deltat);
...
indexbig=gbig.index(xindex,yindex,0);
result+=wf[indexbig]*start[indexbig]*gbig.delt_y()*gbig.delt_x();
};
};
